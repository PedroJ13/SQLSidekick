...
-- title: Recent runs
-- description: Recent SQL Agent job and step outcomes summarized as process runs.
BEGIN TRY
    SELECT TOP (1000)
        j.name AS process_name,
        h.step_id,
        CASE WHEN h.step_id = 0 THEN '(Job outcome)' ELSE js.step_name END AS step_name,
        CASE h.run_status
            WHEN 0 THEN 'Failed'
